...
test.describe("home-main controls", () => {
  test("home-main ctas should be clickable", async ({ page }) => {
    await locateClickStore(page, "main", "link", "Accessibility 101");
    await expect(
      page.getByRole("heading", { name: "Accessibility 101" })
    ).toBeVisible();

    await locateClickStore(page, "main", "link", "Check for bugs");
    await expect(
  ...
  test("home-we-can-help links should be clickable", async ({ page }) => {
    await locateClickStore(page, "main", "link", "Your portfolio site");
    await expect(
    ...
    await locateClickStore(page, "main", "link", "LinkedIn opens in a new tab");
  ...
  test("home-a11y-101 links should be clickable", async ({ page }) => {
    await locateClickStore(page, "main", "link", "The most common bugs");
    await expect(
      page.getByRole("heading", { name: "The most common bugs" })
    ).toBeVisible();
    await page.goBack();

    await locateClickStore(page, "main", "link", "What is accessibility?");
    await expect(
      page.getByRole("heading", { name: "What is accessibility?" })
    ).toBeVisible();
    await page.goBack();

    await locateClickStore(page, "main", "link", "How accessibility works");
    await expect(
